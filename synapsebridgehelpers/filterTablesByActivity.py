...
def filterTablesByActivity(syn, tables, healthCodes=None):
    ...
    if healthCodes is None:
        table_activities = dict(tables.groupby(by='simpleName')['id'].apply(list))
    else:
        res = synapsebridgehelpers.find_tables_with_data(syn=syn,
                                                         healthCodes=healthCodes,
                                                         tables=tables)
        res = res[res['healthCodeCounts'] > 0]
        table_activities = dict(res.groupby(by='simpleName')['id'].apply(list))
