...
def groupTableActivity(syn,sourceProjId, extIdStr = '', activityNameFilters=[], healthCodes = None):
    ...
    if extIdStr == '' and healthCodes == None:
        return table_activities
    elif healthCodes == None:
        ...
        res = synapsebridgehelpers.find_tables_with_data(syn=syn, healthCodes=healthCodes, tables=all_tables)
        res = res[res['healthCodeCounts']>0]
        table_activities = dict(res.groupby(by='simpleName')['table.id'].apply(list))
        return table_activities
